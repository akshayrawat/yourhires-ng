    ...
    def auto_complete_for(object, method, options = {})
      define_method("auto_complete_for_#{object}_#{method}") do
        ...
        find_options = { 
          ...
          :order => "#{method} ASC",
          :select => "#{object_constant.table_name}.id, #{object_constant.table_name}.#{method}",
          :limit => 10 }.merge!(options)
        
        @items = object_constant.find(:all, find_options).collect{|item|
"#{item.id}|#{item.send(method)}"}
